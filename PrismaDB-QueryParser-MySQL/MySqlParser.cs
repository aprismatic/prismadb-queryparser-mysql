...
namespace PrismaDB.QueryParser.MySQL
{
    ...
    {
        private readonly Grammar _grammar = new MySqlGrammar();
                ...
                if (node != null)
                    if (node.Term.Name.Equals("stmtList"))
                        foreach (var stmtNode in node.ChildNodes)
                            ...
                            {
                                var registerCommand =
                                    new RegisterUserCommand(FindChildNode(stmtNode, "string").Token.ValueString, FindChildNode(stmtNode, "string", 1).Token.ValueString);
